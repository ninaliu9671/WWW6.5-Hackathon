...
## 🚀 Demo 演示场景：WoofCoin (WOOF)

在本次黑客松 Demo 中，我们模拟了一个典型的“土狗币”诱惑场景：

1. **诱惑：** 测试链上部署了暴涨的虚假 Meme 币 `$WOOF`。
2. **锁定：** 用户在冷静期质押 100 USDC，设定针对 `$WOOF` 的 禁买契约。
3. **违约：** 用户受诱惑在 DEX 买入 `$WOOF`。
4. **惩罚：** 协议瞬间执行 Slash，质押金转入奖励池。
5. **结局：** `$WOOF` 随后 Rug Pull 归零。协议虽扣除押金，却帮助用户规避了更大的本金归零风险。
